<template>
  <div v-show="addresses.length">
    <button class="button is-runebase" @click="show = !show">
    ...
    <table v-show="show">
      <tbody>
        <tr v-for="{address, balance} in list">
          ...
          <td class="monospace has-text-right">{{ balance | runebase(8) }}</td>
          <td>
      ...
      <tfoot v-if="addresses.length > 1">
        <tr>
          ...
          <td class="monospace has-text-right">{{ totalBalance | runebase(8) }}</td>
          <td></td>
        </tr>
      </tfoot>
    </table>
    ...
    methods: {
      ...
      async onTransaction({address, id}) {
        ...
        if (window.Notification && Notification.permission === 'granted') {
          ...
          let options = {
            body: id,
            icon: 'https://upload.wikimedia.org/wikipedia/commons/thumb/5/51/Runebase_logo.svg/102px-Runebase_logo.svg.png',
            data: id,
            tag: 'transaction/' + id
          }
